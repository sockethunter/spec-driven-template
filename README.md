...
> ⚠️ Do **not** edit `docs/Tasks.md` manually – it's auto-managed by Claude.
...
.
├── .vscode/
│   ├── extensions.json       # RunOnSave config
│   └── settings.json         # Hook definitions
├── docs/
│   ├── Spec.md               # Specs → edited by humans
│   ├── Bugs.md               # Bug reports → edited by humans
│   └── Tasks.md              # Generated by Claude
├── hooks/
│   ├── spec-driven           # Core logic for task mapping
│   ├── spec-driven-impl      # Implements tasks from Spec.md
│   ├── spec-driven-bug       # Fixes bugs from Bugs.md
│   └── docgen/
│       ├── kdoc              # Adds KDoc to Kotlin code
...
- 🕒 Debouncing to prevent rapid re-execution
- 🧠 Caching to avoid redundant actions
- 🔒 PID management to avoid conflicts
- 💻 VSCode status integration
- ⏱️ Timeout handling for stability
- 🌐 Supports multiple languages (Kotlin, JS/TS)
...
- 🧹 Wipe old docs/ content
- 🆕 Recreate minimal Spec.md and Bugs.md
- ✅ Let Claude restart from a clean base
